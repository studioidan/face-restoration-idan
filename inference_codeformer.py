...
def start(image_path):  # device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    device = get_device()
...
def start_with_data(image_data):
    if not os.path.exists(tmp_dir):
        os.makedirs(tmp_dir)
    file_name = f'{int(time.time())}.png'
    image_path = f'{tmp_dir}/{file_name}'
...
def original():  # device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    device = get_device()
    parser = argparse.ArgumentParser()
